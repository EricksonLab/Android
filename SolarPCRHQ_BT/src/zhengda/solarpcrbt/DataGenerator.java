...
public class DataGenerator {
	...
	public void getData(Bitmap CurGraph){
		...
		for (int j=0;j<Width;j++){
			...
			for (int i=0;i<Height;i++){
				...
				if ((HSV[0]>79.2f) && (HSV[0]<180.0f)){
					brightness = (HSV[2]*HSV[1]*(-Math.abs(HSV[0]-129.6f)/50.4f + 1.0f));
					TotalInten = TotalInten + (double) brightness;
				}
			}
	...
	public static void setNewBar(int x){
		...
		float step = (float)(ValueSet.RightEdgeX - ValueSet.LeftEdgeX) / 4.0f;
